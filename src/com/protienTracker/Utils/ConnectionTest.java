...
public class ConnectionTest {
	public static void main(String[] args) {
		...
		//adding data filter to session level
		session.enableFilter("nameFilter").setParameter("name", "j%");
		
		session.beginTransaction();
		Users user=new Users();
		user.setName("Simranjit");
		user.getHistory().add(new UserHistory(new Date(),"setting name to simran"));

		user.getProteinData().setGoal(300);
		user.getProteinData().setTotal(user.getProteinData().getTotal()+50);
		user.getHistory().add(new UserHistory(new Date(),"setting goal to 300"));
